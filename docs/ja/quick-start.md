...
ファイルパネルでファイルをクリックすると、ターミナルの横のエディタで開きます。シンタックスハイライト、元に戻す・やり直す、**⌘S** での保存、**⇧⌥F** でインストール済みフォーマッタによる整形に対応します。エージェントが変更したファイルは自動で読み直されます。マークダウンファイルには **プレビュー** ボタン（**⇧⌘V**）があり、VS Code のプレビューと同じ方法で表示します。中のリンクはブラウザで、ファイルへのリンクはエディタで開きます。プレビューは現在 macOS のみです。
