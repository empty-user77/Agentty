...
### 브랜치 칩: ⌘ 클릭으로 웹에서 열기 · 상태 색

| 항목 | 내용 |
|---|---|
| ⌘ 클릭 | 창 헤더의 브랜치 칩을 ⌘ 클릭하면 `origin`의 웹 주소에서 그 브랜치 페이지를 엶(`git::branch_web_url`: GitHub `/tree/<branch>`, GitLab `/-/tree/`, Bitbucket `/src/`, Gitea · Codeberg `/src/branch/`, 그 외는 GitHub 형식). 브랜치 이름의 `/`는 유지하고 `#` · 공백 등은 이스케이프. 아직 푸시하지 않은 브랜치는 404가 되므로 저장소 페이지를 열고 상태 줄에 안내, 원격이 없으면 안내만. 링크 열기 설정(내장 브라우저 / 기본 브라우저)을 따름. 일반 클릭은 기존 브랜치 메뉴 그대로, 툴팁에 `⌘ click → web` |
| 상태 색 | 수정 있음 = 노랑(`*` 표시와 같은 색), 푸시할 커밋 있음 = 옅은 파랑(`↑N`과 같은 색), 둘 다 없으면 기존 회색. 둘 다면 먼저 처리할 "수정 있음"이 우선 |
| 같이 고친 것 | 좁은 창에서 헤더의 폴더 이름이 브랜치 칩 위로 겹치던 문제(폴더 이름이 줄어들도록) |
| 훅 보완 | `require-security-audit.py`가 다른 저장소(테스트용 스크래치 저장소)의 커밋까지 막던 문제: `git -C <dir>` 또는 `cd <dir>`로 이 저장소 밖을 가리키는 것이 명령에서 읽히면 대상에서 제외. 변수 · 치환 · `--git-dir` · `GIT_DIR`처럼 읽을 수 없는 경우는 이 저장소로 간주(닫힌 쪽으로 실패). 15개 사례로 확인 |
| 검증 | ✅ 디버그 스냅샷: 깨끗함 / ahead / dirty 저장소에서 회색 / 파랑 / 노랑 · ⌘ 클릭 시 내장 브라우저에 `https://github.com/empty-user77/Agentty/tree/feat/idea-launch` 열림 · 좁은 헤더에서 겹침 없음 · `branch_pages_by_host` 테스트 |

### 완료 알림에 답변 첫 줄

| 항목 | 내용 |
|---|---|
| 증상 | Claude Code 창의 완료 알림이 전부 "작업 완료" |
| 원인 | 알림 문구는 훅이 준 `message` / `last_assistant_message`를 쓰는데, Claude Code의 `Stop` 훅은 답변 내용을 주지 않음(Codex의 notify는 줌) → 항상 기본 문구로 폴백 |
| 수정 | `Stop`에 메시지가 없으면 세션 기록에서 **마지막 답변의 첫 줄**을 읽어 알림 · 시스템 알림 · 미니 패널 말풍선 · 창 상태에 사용(`claude::last_reply_headline`: 서브에이전트 답변 제외, 제목 · 목록 · 인용 기호와 `**` · 백틱 제거, 기호뿐인 줄은 건너뜀, 140자). 기록이 없으면 기존 문구. 손으로 띄운 Claude 세션(훅 없음)의 완료 감지에도 같은 문구 |
| 검증 | ✅ 단위 테스트(마크다운 정리 · 서브에이전트 제외 · 길이) · 이 세션의 실제 기록에 같은 규칙 적용 시 "⌘ 클릭으로 브랜치를 GitHub에서 여는 기능과 …" |
| ⚠️ | 실제 앱에서 Claude 턴을 끝내 알림을 받아 보는 확인은 아직 · 답변 첫 줄이 macOS 알림 센터에도 표시됨(잠금 화면 노출은 시스템 알림 설정에 따름) |
