...
| `media/htmlToVideo` | `files` | `{ from, to, seconds, width?, height?, fps? }` | `{ width, height, seconds, frames, size }` — records the HTML page `from` (the plugin's folder, at most 2 MB) into an H.264 MP4 at `to` (default 1920×1080, 30 fps, at most 60 s). The page loads out of sight with no network (a CSP allows only inline code and `data:`), no cookies and a virtual clock put in before its scripts: `requestAnimationFrame`, timers, `Date`, `performance.now` and `Math.random` follow it, and each frame sets CSS / Web Animations and SVG animations to that moment. Fails on a script error, if the page goes to another address, or if nothing moves; macOS only for now |
