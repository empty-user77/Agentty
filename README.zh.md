...
- **会话之间可以传递上下文** — 在两个智能体之间拉一条连线即可交接对话，保持双向同步，或在 Claude Code 与 Codex
  之间迁移会话。
- **并行工作而不互相干扰** — 同一项目中的第二个智能体会在新分支的独立 git worktree 中启动；智能体请求把工作拆成
  多个会话时，经你确认后一次性开始。
...
官方发布版本会发送匿名使用统计（使用了哪些功能、应用与系统版本、随机安装 ID）。首次启动会展示该设置，可在
...
Agentty 是自由软件：你可以在 [GNU General Public License](LICENSE) 第 3 版或（由你选择）任何更高版本的条款下
重新发布和修改它。
