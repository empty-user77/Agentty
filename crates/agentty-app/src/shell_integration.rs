...
pub fn powershell_script(aliases: &[CommandAlias]) -> String {
    ...
    let mut out = String::from("# Generated by Agentty: reserved words and working trees. Edit words in Settings.\n");
    out.push_str(POWERSHELL_FOLLOW_LOCATION);
    ...
    for (command, entries) in wrapped(aliases) {
        ...
        if AGENTS.contains(&command.as_str()) {
            ...
            let _ = writeln!(out, "  __agentty_sync_dir");
        }
...
/// PowerShell's location (`cd`) is not the process's current folder, which is all Agentty can read
/// from outside. Each prompt copies one into the other, so a pane is saved and split where the user
/// is. The user's own prompt runs first and unchanged (it may read `$?`).
const POWERSHELL_FOLLOW_LOCATION: &str = r#"function global:__agentty_sync_dir {
  try { if ($PWD.Provider.Name -eq 'FileSystem') { [System.IO.Directory]::SetCurrentDirectory($PWD.ProviderPath) } } catch { }
}
if (-not $global:__agentty_prompt) {
  $global:__agentty_prompt = if ($function:prompt) { $function:prompt } else { { "PS $($executionContext.SessionState.Path.CurrentLocation)> " } }
  function global:prompt {
    $__agentty_text = & $global:__agentty_prompt
    __agentty_sync_dir
    $__agentty_text
  }
}
...
mod tests {
    ...
    #[test]
    fn powershell_prompt_follows_the_location() {
        let script = powershell_script(&[]);
        assert!(script.contains("function global:prompt {") && script.contains("SetCurrentDirectory($PWD.ProviderPath)"));
        // An agent typed after `cd` starts in the folder the pane shows, prompt or not.
        assert!(script.contains("__agentty_own_tree 'claude' $args\n  __agentty_sync_dir\n"));
    }

    /// `cd` in real Windows PowerShell moves the process's folder at the next prompt, which is
    /// what Agentty reads to save and split a pane.
    #[test]
    #[cfg(windows)]
    fn powershell_prompt_moves_the_process_folder() {
        let target = std::env::temp_dir().canonicalize().unwrap();
        let script = format!(
            "{}\nSet-Location -LiteralPath '{}'\n$null = prompt\n[System.IO.Directory]::GetCurrentDirectory()",
            powershell_script(&[]),
            target.display().to_string().trim_start_matches(r"\\?\").replace('\'', "''")
        );
        let output = agentty_bridge::process::windows_powershell()
            .args(["-NoProfile", "-NonInteractive", "-EncodedCommand", &crate::launch::encode_powershell(&script)])
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .output()
            .unwrap();
        let out = String::from_utf8_lossy(&output.stdout);
        let reported = std::path::PathBuf::from(out.trim()).canonicalize();
        assert_eq!(reported.ok(), Some(target), "{out}{}", String::from_utf8_lossy(&output.stderr));
    }
